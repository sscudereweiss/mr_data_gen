...
function service_kpi_backfill(){
    ...
    response=$(curl -s -k -u "${USERNAME}:${SPLUNK_PASSWORD}" "$ITSI_API_URL")
    service_title=$(echo "$response" | jq '.title' | sed 's/"//g')
    while [[ $result < 8 ]]; do
      result=$(sudo /opt/splunk/bin/./splunk search "| rest /services/messages | search message=\"Backfill*${service_title}*completed*\" | stats count" -auth ${USERNAME}:${SPLUNK_PASSWORD} -header false  2>/dev/null) 
    ...
    curl -k -u "${USERNAME}:${SPLUNK_PASSWORD}" https://${SPLUNK_HOST}:8089/servicesNS/nobody/SA-ITOA/itoa_interface/service/bulk_update/?is_partial_data=1 -H "Content-Type: application/json" -X POST -d "$kpi_keys" 
}
