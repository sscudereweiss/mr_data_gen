...
if [ -r "$file" ]; then
     while IFS= read -r line; do
          ...
          for ((i=1; i<=$minutes_backfill; i++)); do
               ...
               Network_Bytes_Received=$(shuf -i 100-1000000 -n 1 # Random Bytes received/sec between 1MB and 11MB
               Network_Bytes_Sent=$(shuf -i 100-600000 -n 1) # Random Bytes sent/sec between 1MB and 6MB
               curl -k -s -o /dev/null https://localhost:8088/services/collector -H "Authorization: Splunk REDACTED_HEC_TOKEN" -d '{ "time": "'${back_time}'", "event": "metric", "source": "mcollect", "sourcetype": "mcollect_stash", "host": "'${line}'", "fields": { "metric_name:cpu.idle": "'${CPU_Idle}'", "metric_name:memory.used": "'${Memory_Usage}'", "metric_name:df.used": "'${Disk_Free}'", "metric_name:disk.ops.read": "'${Disk_Read}'", "metric_name:disk.ops.write": "'${Disk_Write}'", "metric_name:interface.octets.rx": "'${Network_Bytes_Received}'", "metric_name:interface.octets.tx": "'${Network_Bytes_Sent}'"}}' &
          done
     done < "$file"
else
