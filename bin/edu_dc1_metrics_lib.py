"""EDU DC1 metric generation library for scripted input."""
...
def get_app_dir() -> str:
    ...
    if os.path.isfile(os.path.join(repo_candidate, "lookups", "edu_dc1_hosts.csv")):
...
def load_settings(app_dir: Optional[str] = None) -> configparser.ConfigParser:
    app_dir = app_dir or get_app_dir()
    parser = configparser.ConfigParser()
    for path in (
        os.path.join(app_dir, "default", "edu_dc1_datagen.conf"),
        os.path.join(app_dir, "local", "edu_dc1_datagen.conf"),
    ):
...
def load_hosts(
    ...
    lookup_name: str = "edu_dc1_hosts.csv",
...
def iter_host_lines(
    ...
    app_dir = get_app_dir()
    lookup_name = settings.get("settings", "hosts_lookup", fallback="edu_dc1_hosts.csv")
